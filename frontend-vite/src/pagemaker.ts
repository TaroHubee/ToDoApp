import { EditorDatabaseManeger } from "./databaseManeger.js";
...
export class PageMaker {
    ...
    createAddForm() {
        const databaseManeger = new EditorDatabaseManeger(this.apiURL)
        ...
        addNameButton.addEventListener("click", async() => {
            const message = await databaseManeger.postRows(addNameForm.textContent!);
